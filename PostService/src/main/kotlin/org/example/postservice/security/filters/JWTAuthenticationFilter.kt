package org.example.postservice.security.filters
    ...
    override fun doFilter(request: ServletRequest, response: ServletResponse, chain: FilterChain) {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                val authentication = PostTokenDTO(
                    claims["username"] as String,
                    authorities,
                    role
                )

                // Extract friends from JWT claims and add to the request
                val friends = claims["friends"] as? List<Map<String, String>>
                val friendUsernames = friends?.mapNotNull { it["username"] } ?: emptyList()
                request.setAttribute("friendUsernames", friendUsernames)
